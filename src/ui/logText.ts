...
export function describeMove(state: GameState, seat: Seat | null, move: Move): string {
  ...
  switch (move.type) {
    ...
    // the posts turn over every round, so naming today's post would relabel an older entry; the engine's own
    // info entry right after this one names the post and the ability as they were
    case 'postAbility': return `${name} uses the ${move.post} trade post's special ability`
