...
- Taking a command token back out of the fleet pool is a legal move, not an error. Whenever the pool shrinks, every system where that player now has more non-fighter ships than the pool allows destroys ships until it fits: the cheapest ships go first, they return to reinforcements like any other loss, and each loss is logged with its system. Fighters and infantry that lose the transport they were riding are trimmed by the capacity rules. The moves that can shrink the pool are the Warfare primary's redistribution and the charter and the layover at the trade posts; the status phase only hands out new tokens (section 3.3 step 3) and never moves one that is already on the sheet. The player is warned what a sheet costs before they confirm it, and never blocked.
- Trade goods and commodities (commodity value 2 for both factions).
- Objectives: six public objectives drawn from a pool at setup, one revealed per round, worth 1 VP each, two of them paid for when they are scored, plus the two mandates (section 7).
...
- Chess clock: 15 minutes per player, running whenever it is that player's turn to decide something, in every phase: picking a strategy card, taking an action, answering a secondary, distributing status tokens. It stops only while the handoff screen is up and once the game is over, so neither player can hold the other one hostage by sitting on a draft pick. At zero the player automatically passes for the rest of the round and receives 3 extra minutes at the start of each later round. The engine is time-free; the transport records a timestamp per move (see lobby-architecture.md) and enforces the clock. A rule whose price is clock time (R7, "pay a fifth of the time left on your clock") is therefore settled by the client too: the engine grants the point and logs it, and the side holding the clock subtracts the time when it applies the move.

## 7. Objectives and victory
- Public objectives: a pool the game draws six from at setup, with the game seed. One is on the table from the first round and one more is revealed after each status phase, so the six run out with round 6, which reveals nothing itself. Which six are in play, and in which order, changes from game to game; the rest of the pool stays out of that game, and a replay of a seed draws the same six in the same order. The pool: win a space combat against your opponent; control 4 planets outside your home system; pay 6 resources when you score this; trade three times, at the trade posts or with your opponent; have more ships on the board than your opponent (every ship type counts); own 5 technologies (unit upgrades and faction technologies count, exactly as `player.techs` holds them); pay a fifth of the time left on your clock. Each is worth 1 VP, scored once per player in a status phase.
- Paying for an objective. Most objectives score themselves in the status phase as soon as they are fulfilled. Two have a price, and those are scored only when the player asks for them in their status move and covers the cost there. "Pay 6 resources when you score this" is fulfilled whenever the player could raise 6 out of ready planets and trade goods, and scoring it exhausts what they named; overpay is lost, as everywhere else. "Pay a fifth of the time left on your clock" is always fulfilled while revealed and unscored; the engine grants the point and logs the debt, and the client that owns the clock takes 20 percent of that seat's remaining time, rounded down to the second, because the engine is time-free. An unrequested or unpayable paid objective is simply not scored, and nothing is taken for it. The Imperial primary refuses to score a paid objective: its point is free, and free is not the price.
