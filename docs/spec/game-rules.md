...
### 4.4 Production
A space dock produces up to (planet resources + 2) units (Space Dock II: +4, and 3 fighters do not need capacity). Costs from the unit table; fighters and infantry come in pairs for their cost. Payment: exhaust ready planets (resources) and spend trade goods (1 each); overpay is lost. Sarween Tools: total cost -1 (minimum 0). Fleet pool: after production, non-fighter ships in the system may not exceed the fleet pool (Armada: +2); fighters need capacity of ships in the system (or Space Dock II slots), excess fighters are not produced (the move succeeds with the trimmed count); non-fighter ships beyond the fleet pool make the production illegal. War Sun requires the War Sun technology; flagship: one per player at a time.
